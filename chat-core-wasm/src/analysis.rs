...
#[cfg(all(target_arch = "wasm32", feature = "timing"))]
fn log_step(label: &str, start: f64) {
    web_sys::console::log_1(&format!("[wasm] {} took {:.1}ms", label, perf_now() - start).into());
}
...
impl<'a> Drop for TimingGuard<'a> {
    fn drop(&mut self) {
        log_step(self.label, self.start);
    }
}
...
mod tests {
    ...
    use crate::{metrics, parsing, phrases, text, Count};
